...
当我们将值7赋给变量width时，先前存在的值5将被覆盖。变量值只能保存一次。
...
赋值的一个特点是它至少需要两个语句：用于定义变量，用于分配值。
...
最初引入直接初始化是为了更有效地初始化复杂对象（那些具有类类型的对象）。由于被列表初始化所取代，直接初始化在现代C++中已经不受欢迎，就像拷贝初始化一样。然而，列表初始化本身有一些奇怪的特性，因此直接初始化在某些情况下再次得到了应用。
...
为了更多地讨论这个主题，Bjarne Stroustrup（C++的创建者）和Herb Sutter（C++专家）在这里自己提出了建议。
