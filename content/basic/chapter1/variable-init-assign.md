...
赋值的一个特点是它至少需要两个语句：一个用于定义变量，另一个用于分配值。
...
您可能会发现上面的花括号以不同的间距编写。是否使用额外的空格来提高可读性，是个人偏好问题。
...
最初引入直接初始化是为了更有效地初始化复杂对象（那些具有类类型的对象）。就像拷贝初始化一样，直接初始化在现代C++中已经不受欢迎，这主要是由于被列表初始化所取代。然而，列表初始化本身有一些奇怪的特性，因此直接初始化在某些情况下再次得到了应用。
...
列表初始化还有一个额外的好处：它不允许“缩小转换范围”。这意味着，如果尝试使用变量不能安全保存的值，编译器将产生错误。例如：
...
拷贝和直接初始化只会删除小数部分，从而将值4初始化到变量width（编译器可能会就此发出警告，因为很少需要丢失数据）。然而，使用列表初始化时，编译器将生成错误，迫使您纠正此问题。
...
我们还注意到，最佳实践是完全避免这种语法。然而，由于您可能会遇到使用这种风格的其他代码，那么再多谈谈它仍然是有用的。
...
不幸的是，当程序员尝试使用一个初始化语句来初始化两个变量时，可能会出现一个常见的陷阱：
...
在顶部语句中，变量“a”将保持未初始化状态，编译器可能会告警，也可能不会告警。这是一种让程序可能间歇性崩溃的好方法。
