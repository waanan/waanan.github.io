...
在第5.7课——内联函数和变量中，我们介绍了内联变量，这些变量可以有多个定义，只要这些定义相同。通过使constexpr变量内联，我们可以在头文件中定义它们，然后将它们#包含到需要它们的任何.cpp文件中。这避免了ODR冲突和重复变量的缺点。
...
对constexpr字符串使用std:：string_view。我们在第5.10课——std:：string_view简介中介绍了这一点。
