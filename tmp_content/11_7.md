...
类型推断（有时也称为类型推断）是一种允许编译器从对象的初始值设定项推断对象类型的功能。要对变量使用类型演绎，请使用auto关键字代替变量的类型：
...
由于函数调用是有效的表达式，因此当初始值设定项是非void函数调用时，我们甚至可以使用类型演绎：
...
类型演绎将不适用于没有初始值设定项或具有空初始值设定值的对象。当初始值设定项具有类型void（或任何其他不完整类型）时，它也将不起作用。因此，以下内容无效：
...
如果希望从字符串文字推导的类型为std:：string或std:∶string_view，则需要使用s或sv文字后缀（在第5.2课--文字中介绍）：
...
变量无符号时也会发生类似的情况。由于我们不想混合有符号值和无符号值，因此显式地知道变量具有无符号类型通常是不应该混淆的。
...
如果add的返回类型从int变为double，或者重力从int变为了double时，sum的类型也将从int变到double。
...
当对象的类型无关紧要时，对变量使用类型演绎。

当您需要与初始值设定项的类型不同的特定类型时，或者当您的对象在使类型显而易见的上下文中使用时，请支持显式类型。
